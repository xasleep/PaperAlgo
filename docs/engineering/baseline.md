...
## 2026-09-03 PR-07A 验收快照

- python -m compileall -q codes web_api tests: 通过。
- PR-07A SSE 和 persisted command 定向测试: 18 passed。
- python -m pytest tests -q -rs: 633 passed。
- WebUI typecheck、build、verify:same-origin、smoke、smoke:prod: 全部通过。
- Python 测试仍只报告 1 个既有 StarletteDeprecationWarning; 未运行真实 LLM、MinerU、vLLM、公共网络队列或完整 Pipeline。
